...
func _ready() -> void:
    ...
    # Keep every menu/result/failure overlay above the in-level HUD.
    ...
    if not EventBus.restart_requested.is_connected(_restart_current_level):
        EventBus.restart_requested.connect(_restart_current_level)
    if not EventBus.main_menu_requested.is_connected(show_main_menu):
        EventBus.main_menu_requested.connect(show_main_menu)
    if not EventBus.challenge_failed.is_connected(_on_challenge_failed):
        EventBus.challenge_failed.connect(_on_challenge_failed)
    show_main_menu()

func _exit_tree() -> void:
    if EventBus.level_completed.is_connected(_on_level_completed):
        EventBus.level_completed.disconnect(_on_level_completed)
    if EventBus.restart_requested.is_connected(_restart_current_level):
        EventBus.restart_requested.disconnect(_restart_current_level)
    if EventBus.main_menu_requested.is_connected(show_main_menu):
        EventBus.main_menu_requested.disconnect(show_main_menu)
    if EventBus.challenge_failed.is_connected(_on_challenge_failed):
        EventBus.challenge_failed.disconnect(_on_challenge_failed)
...
func _load_level(index: int) -> void:
    ...
    if current_level == null:
        push_error("Level root is not a LevelManager: " + path)
        transition_in_progress = false
        show_main_menu()
        return
...
func _on_challenge_failed(reason: String) -> void:
    if (
        ...
        return
    var scene := load("res://scenes/ui/fail_screen.tscn") as PackedScene
    if scene == null:
    ...
    fail_screen.retry_requested.connect(_restart_current_level)
    fail_screen.main_menu_requested.connect(show_main_menu)

func _on_level_completed(id: String) -> void:
    if current_level == null or results_open or transition_in_progress:
        return
    ...
    if (
        ...
        return
    var scene := load("res://scenes/ui/results_screen.tscn") as PackedScene
    if scene == null:
    ...
    results.main_menu_requested.connect(show_main_menu)

func _show_pause() -> void:
    if pause_menu != null or transition_in_progress or results_open:
        return
...
func _create_ending() -> void:
    var scene := load("res://scenes/ui/ending_screen.tscn") as PackedScene
    if scene == null:
